...
import React from "react";
import { useForm } from "react-hook-form";
import "@/components/css/Services.css"; // Importing the new CSS file
...
const Services = () => {
  ...
  async function onSubmit(data) {
    // Added type 'any' for data for simplicity
    console.log("Submitting: ", data);
    try {
      const carDetails = {
        manufacturer: data.manufacturer,
        model: data.models, // Note: 'models' in data, 'model' in carDetails
        condition: data.condition, // Corrected from 'cylinders'
        cylinders: data.cylinders,
        fuel: data.fuel,
        odometer: data.odometer,
        title_status: data.title_status,
      ...
      console.log("Car Details for API:", carDetails); // Log the structured data
      ...
      // IMPORTANT: This fetch URL is for a local Flask server.
      // It will not work in the v0 preview environment.
      // You will need to run your Flask server locally for this to function.
      const res = await fetch("http://127.0.0.1:5000/", {
        method: "POST",
        body: formData,
      });
      ...
      console.error("Failed to get prediction:", error); // Use console.error for errors
      alert("Failed to get prediction. Please try again.");
    }
  }
        ...
        <div className="form-group">
          ...
          <select
            ...
            {...register("paint_color", {
              required: "Paint color is required",
            })}
          >
            <option value="">Select Paint Color</option>
            <option value="grey">Grey</option>
